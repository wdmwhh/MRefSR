...
class ContrasMultiExtractorSep(nn.Module):

    def __init__(self):
        super(ContrasMultiExtractorSep, self).__init__()

        self.feature_extraction_image1 = ContrasExtractorLayer()
        self.feature_extraction_image2 = ContrasExtractorLayer()

    def forward(self, image1, image_list):
        dense_features1 = self.feature_extraction_image1(image1)
        dense_feats_list = [self.feature_extraction_image2(image2) for image2 in image_list]

        return [{
            'dense_features1': dense_features1,
            'dense_features2': dense_features2
            }
            for dense_features2 in dense_feats_list
        ]
