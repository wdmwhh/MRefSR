...
class MultiRefRestorationModel(BaseModel):

    def __init__(self, opt):
        super(MultiRefRestorationModel, self).__init__(opt)

        # net_map does not have any trainable parameters.
        self.net_map = build_network(opt['network_map']).cuda()
    ...
    def optimize_parameters(self, step):
        feats_list = self.net_extractor(self.match_img_in, self.img_ref_list)
        pre_offset_list = []
        img_ref_feat_list = []
        for features, img_ref in zip(feats_list, self.img_ref_list):
            pre_offset, img_ref_feat = self.net_map(
                features, img_ref)
            pre_offset_list.append(pre_offset)
            img_ref_feat_list.append(img_ref_feat)
        self.output = self.net_g(self.img_in_lq, pre_offset_list,
                                 img_ref_feat_list)
